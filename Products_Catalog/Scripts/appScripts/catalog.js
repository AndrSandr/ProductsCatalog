...
function AddProduct() {
    ...
    if (!checkDigitsInput($('#addPrice').val())) {
        alert('Only numbers can be entered in a price.');
        return;
    }
    ...
    if (!window.File || !window.FileReader) { //(!window.File || !window.FileReader || !window.FileList || !window.Blob) {
        alert('File API does not support in your browser.');
        $('#addPhoto').val('');
    }
...
function checkBiggestPrice(price) {
    if (price > 999) {
        return confirm('You entered a price greater than 999. Save this price?');
    } else {
        return true;
    }
}